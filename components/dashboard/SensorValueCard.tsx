...
const statusStyles = {
  normal:   { card: "border-slate-200 dark:border-slate-700/60", dot: "bg-green-400",              label: "正常", labelColor: "text-green-600", bar: "bg-green-400" },
  ...
  fault:    { card: "border-slate-200 dark:border-slate-700/60", dot: "bg-slate-300",               label: "故障", labelColor: "text-slate-400 dark:text-slate-500", bar: "bg-slate-300" },
};
...
export function SensorValueCard({ type, name, value, unit, minNormal, maxNormal, healthScore, lastReadAt }: SensorValueCardProps) {
  ...
  return (
    <div className={cn("rounded-xl border bg-white dark:bg-slate-900 p-4 transition-all", styles.card)}>
      ...
      <p className="text-xs text-slate-400 dark:text-slate-500 mb-0.5">{name}</p>
      <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 leading-none tabular-nums">
        {value !== null ? value.toFixed(2) : "—"}
        <span className="text-xs font-normal text-slate-400 dark:text-slate-500 ml-1">{unit}</span>
      </p>
      <div className="mt-3 h-1 rounded-full bg-slate-100 dark:bg-slate-800">
